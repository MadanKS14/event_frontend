...
import {
  ...
  // Removed unused icons like Moon, Sun, Bot, LayoutGrid
} from "lucide-react";
...
export const UserDashboard = () => {
  ...
  // Load data and setup socket on component mount
  useEffect(() => {
    loadEvents();
    const newSocket = io(SOCKET_URL, { transports: ["websocket", "polling"] });
    newSocket.on("connect", () => console.log("WebSocket connected"));
    newSocket.on("task-updated", loadEvents); // Refresh list if user's tasks change
    setSocket(newSocket);
    return () => newSocket.disconnect();
  }, []); // Empty dependency array ensures this runs only once on mount

  // API call function for user (backend filters automatically)
  const loadEvents = async () => {
    ...
    finally { if (loading) setLoading(false); } // Only stop initial loading spinner
  };

  // Event action handler (opens details modal)
  const handleEventClick = (event) => { setSelectedEventId(event._id); };

  // Process events (filter/sort for user view)
  const processedEvents = useMemo(() => {
    ...
    return processed;
  }, [events, filterStatus, sortOrder]); // Recalculate when these change
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-cyan-50/30 dark:from-gray-900 dark:to-gray-800 transition-colors">
      ...
      <Navbar
        ...
        // Pass dummy/empty values for props Navbar expects but user doesn't use
        searchTerm=""
        onSearchChange={() => {}} // No-op function as users don't search globally
        onToggleAI={() => setShowAI(prev => !prev)} // Pass AI toggle function
      />
      {/* --- End Navbar --- */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          {/* Controls for User View (Filter/Sort/View Toggle) */}
        ...
        {/* --- Event Display Area (using processedEvents) --- */}
        {view === "grid" ? (
          processedEvents.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Map over processed events */}
              {processedEvents.map((event) => (
                <EventCard
                    key={event._id}
                    event={event}
                    onClick={handleEventClick}
                    isUser={true} // Pass isUser=true to hide admin controls on card
                 />
              ))}
            </div>
          ) : (
             ...
             <div className="text-center py-16">
               ...
               <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                 {/* Message depends on whether filters are active */}
          ...
          // Calendar View uses processed events
      ...
      <EventDetailsModal
        ...
        isUser={true} // Pass isUser=true to show user-specific view in modal
      />
      <AIAssistant
        isOpen={showAI}
        ...
        role="user" // Pass role="user" for AI context
       />
    </div>
  );
};
